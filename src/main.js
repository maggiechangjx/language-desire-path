...
function initWordBase() {
   ...
   let words = [
      ...
      'machinic','desire','ugly','no','chill','not','rock','plant','image','seeing',
...
function pageLeftInteractions() {
   ...
   // input 3 words 
   // store those 3 words on firebase if it doesn't exist
   // update page with those 3 words 
   // remove 3 words that aren't clicked so far
   let sentences = document.getElementsByClassName('logged-sentence');
   let word_btns = document.querySelectorAll('button');
   let post_key;
   ...
   onValue(ref(db, 'all_words'), (snapshot) => {
      ...
      if (click == 0) {
         remove(ref(db, 'all_words/' + word)).then(() => {
            console.log('removeWords() working');
         });
      }
...
function removeWord() {
   ...
   // removes some lines from 'lines' in firebase too 
   ...
   onChildAdded(ref(db, 'all_words'), (snapshot)  => {
      ...
      get(linked_lines_ref1).then((snapshot) => {
         ...
         snapshot.forEach((child) => {
            ...
            console.log('child key: ' + line_key);
